        ...
        try {
            for (Cineplex cineplex : arrayName1) {
                for (int j = 0; j < numberCinema; j++) {
                    ...
                    input[2] = Integer.toString(arrayName2.get(j).getCinemaCode());
    ...
    public static void updateReviewList(String filePath, ArrayList<Movie> arrayName) throws IOException {
        ...
        try {
            for (Movie movie: arrayName){
                ...
                //Concatenating all ticketID into 1 String
                String ticketID = "";
                for (int k=0; k<movie.getReviews().getReviews().size(); k++){
                    if (k==0) rating += movie.getReviews().getReviews().get(k).getTicketId();
                    ticketID += ";";
                    ticketID += movie.getReviews().getReviews().get(k).getTicketId();
                }
